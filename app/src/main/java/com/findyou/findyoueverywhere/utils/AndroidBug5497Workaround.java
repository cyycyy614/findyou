...
//    private int statusBarHeight;//状态栏高度
    private int statusBarHeight = UIUtils.getStatusHeight(app.getContext());

    private OnKeyboardToggleListener mOnKeyboardToggleListener;
    private boolean mIsKeyboardShowed;

    public interface OnKeyboardToggleListener {
        void onKeyboardShow(int keyboardSize);
        void onKeyboardHide(int keyboardSize);
    }
            ...
            if (heightDifference > (usableHeightSansKeyboard / 4)) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    ...
                    if(mOnKeyboardToggleListener != null){
                        mOnKeyboardToggleListener.onKeyboardShow(heightDifference);
                ...
                int heightDifference1 = contentHeight - usableHeightNow; //添加
                if(mOnKeyboardToggleListener != null && mIsKeyboardShowed){
                    mIsKeyboardShowed = false;
                    mOnKeyboardToggleListener.onKeyboardHide(heightDifference1);
                }
            }
            //7､ 重绘Activity的xml布局
