...
public class BirthdayPopWindow extends BasePopWindow implements OnWheelChangedListener {
    ...
    public BirthdayPopWindow(Activity context, String birthday){
        ...
        if(TextUtils.isEmpty(birthday)){
            Time time = new Time();
            time.setToNow();
            this.birthday = String.format("%04d-%02d-%02d",time.year, time.month, time.monthDay);
        }else {
    ...
    public void tv_save_Clicked(View view) {
        mMyYear = Integer.parseInt(year[mYear.getCurrentItem()]);
        mMyMonth = Integer.parseInt(month[mMonth.getCurrentItem()]);
        mMyDay = Integer.parseInt(day[mDay.getCurrentItem()]);
